...
namespace cuev::debug {

/// Print a symmetric tridiagonal matrix T from its device diagonal d and subdiagonal e.
template <typename T>
void print_tridiag(const char *label, const T *d_d, const T *d_e, int n,
                   cudaStream_t stream) {
...
void print_hh_vecs(const char *label, const T *d_H, int n, cudaStream_t stream) {
    ...
    for (int k = 0; k < n - 1; ++k) {
        std::cout << "  k=" << k << ":";
        for (int s = 0; s < k; ++s) { std::cout << std::setw(9) << ""; }
        std::cout << " [";
...
/// Print a host vector.
template <typename T>
void print_vec(const char *label, const T *h_x, int n) {
    std::cout << "[DEBUG] " << label << ": [";
    for (int i = 0; i < n; ++i) {
...
template <typename T>
void print_mat(const char *label, const T *h_A, int M, int N) {
    std::cout << "[DEBUG] " << label << ":\n";
    for (int i = 0; i < M; ++i) {
        std::cout << "  [";
        for (int j = 0; j < N; ++j) {
