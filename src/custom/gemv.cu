...
template <typename T>
__global__ void gemv_gmem_kernel(T alpha, const T *A, const T *x, T beta, T *y, int M, int N) {
...
__global__ void gemv_smem_kernel(T alpha, const T *A, const T *x, T beta, T *y, int M, int N) {
...
void gemv_gmem(T alpha, const T *A, const T *x, T beta, T *y, int M, int N, cudaStream_t stream) {
    ...
    gemv_gmem_kernel<<<div_up(M, BLOCKSIZE), BLOCKSIZE, 0, stream>>>(alpha, A, x, beta, y, M, N);
}

template <typename T>
void gemv_smem(T alpha, const T *A, const T *x, T beta, T *y, int M, int N, cudaStream_t stream) {
    constexpr int BLOCKSIZE = 256;
    gemv_smem_kernel<T, BLOCKSIZE><<<M, BLOCKSIZE, 0, stream>>>(alpha, A, x, beta, y, M, N);
}
